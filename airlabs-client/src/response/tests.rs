...
fn airlines() {
    ...
    let response = RawResponse::new(AIRLINES_RESPONSE, now.elapsed());
...
fn airports() {
    let now = time::Instant::now();
    let response = RawResponse::new(AIRPORTS_RESPONSE, now.elapsed());
...
fn flight_free() {
    let now = time::Instant::now();
    let response = RawResponse::new(FLIGHT_FREE_RESPONSE, now.elapsed());
